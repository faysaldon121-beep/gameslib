import puppeteerPkg from "puppeteer-core";
...
async function initBrowser() {
  ...
  // IMPORTANT: Force chromium to use the bin shipped in node_modules (NOT __dirname)
  const chromiumBin = `${process.cwd()}/node_modules/@sparticuz/chromium/bin`;
...
export async function extractDownloadLink(slug) {
  const browser = await initBrowser();
  const page = await browser.newPage();

  try {
    await page.goto(`https://ankergames.net/game/${slug}`, {
