...
// build JSON-LD for rich result markup
  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        >
          <Link href="/" className="hover:text-g-purple transition-colors">
            Home
          </Link>
          <span>/</span>
          <Link href="/games" className="hover:text-g-purple transition-colors">
            Games
          </Link>
          <span>/</span>
          <Link
          ...
          <span>/</span>
          <span className="text-g-text truncate max-w-[200px]">
            {game.title}
          </span>
        </nav>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
          ...
          >
            {/* — the site-wide header section has been removed here — */}
            ...
            <section aria-labelledby="about-heading">
              <h2
                id="about-heading"
                className="text-xl font-bold text-g-text mb-3"
              >
                About {game.title}
              </h2>
              <p
                className="text-g-muted leading-relaxed whitespace-pre-wrap"
              ...
              {game.tags?.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-4">
                  {game.tags.map((tag: string) => (
                    <span
                      key={tag}
                      className="flex items-center gap-1 px-3 py-1 text-xs rounded-full bg-g-border text-g-muted hover:text-g-purple cursor-pointer transition-colors"
                    >
                      <Tag size={10} /> {tag}
                    </span>
                  ))}
            ...
            <section aria-labelledby="reviews-heading" id="reviews">
              <h2
                id="reviews-heading"
                className="text-xl font-bold text-g-text mb-2"
              >
                Community Reviews
              </h2>
              <p className="text-g-muted text-sm mb-6">
                Share your experience to help others — and help Google understand
                this game better.
              </p>
              <ReviewList reviews={reviews as any[]} gameTitle={game.title} />
              <div className="mt-8 pt-8 border-t border-g-border">
                <h3 className="text-lg font-bold text-g-text mb-4">
                  Write a Review
                </h3>
              ...
              <dl className="space-y-2 text-sm">
                {[
                  ...
                  >
                    ...
                    <dd className="text-g-text font-medium text-right max-w-[60%]">
                      {String(value)}
                    </dd>
                  </div>
                ))}
